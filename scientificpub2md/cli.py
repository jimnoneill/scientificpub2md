...
from .extract import DEFAULT_DPI, DEFAULT_MODEL, make_backend
...
def build_parser():
    ...
    ap.add_argument("--device", choices=["auto", "cuda", "cpu"], default="auto",
                    help="compute device for the transformers backend (default: auto)")
    ap.add_argument("--backend", choices=["transformers", "vllm"], default="transformers",
                    help="transformers = in-process (CPU/GPU); vllm = OpenAI-compatible server (GPU). Default: transformers")
    ap.add_argument("--model", default=DEFAULT_MODEL, help=f"VLM model id (default: {DEFAULT_MODEL})")
...
def main(argv=None):
    ...
    backend = make_backend(
        args.backend, device=args.device, model=args.model,
        keep_backmatter=args.keep_backmatter, vllm_url=args.vllm_url, vllm_model=args.vllm_model,
    )
    print(f"scientificpub2md {__version__}: {len(pdfs)} PDF(s), backend={args.backend}, "
          f"format={args.format}, device={getattr(backend, 'device', '?')}", flush=True)
    ...
    for i, pdf in enumerate(pdfs, 1):
        ...
        try:
            ...
            doc = format_document(raw, fmt=args.format)
